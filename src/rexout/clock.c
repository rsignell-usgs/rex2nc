...
/*      } else if (GetIndexFromStr (word, PreRel, &index) != -1) {*/
      } else if ((index = ListSearch(PreRel, sizeof(PreRel), word)) != -1) {
...
/*         if (GetIndexFromStr (word, RelUnit, &ans) == -1) {*/
         if ((ans = ListSearch(RelUnit, sizeof(RelUnit), word)) == -1) {
...
/*      } else if (GetIndexFromStr (word, RelUnit, &index) != -1) {*/
      } else if ((index = ListSearch(RelUnit, sizeof(RelUnit), word)) != -1) {
         lenRel++;
...
/*      } else if (GetIndexFromStr (word, AdjDay, &index) != -1) {*/
      } else if ((index = ListSearch(AdjDay, sizeof(AdjDay), word)) != -1) {
         if (index != 1) {
            lenRel++;
            Rel = (relType *) realloc ((void *) Rel,
                                       lenRel * sizeof (relType));
